...
exports.updateProfil = function (req, res) {
   let user = new User(req.body.iduser, req.body.login, req.body.telephone, req.body.password);
   console.log(user);
   mysqlConnection.query("update user SET login='"+req.body.login+"', telephone='"+req.body.telephone+"', password='"+req.body.password+"' where iduser ="+req.body.iduser,
...
exports.profil = function(req, res, next){
	var annonce =  req.session.annonce,
	id = req.session.id;
	...
	if(id == null){
	 ...
	 let sql="SELECT * FROM `users` WHERE `iduser`='"+id+"'";
	   ...
	   mysqlConnection.query(sql, function(err, results){
		   ...
		   res.render('profile.ejs', {annonce:annonce});	  
